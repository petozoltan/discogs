...
```
**Convention**: Links are built using `linkTo(methodOn(ControllerClass.method))` from `org.springframework.hateoas.server.mvc`.
...
**Important note** (line 10-14 in DiscogsApplication.java): Main method runs twice—once by Java and once by Spring Restarter. Don't put initialization logic in `main()`, use `ApplicationRunner` beans instead.
...
1. **No JPA relations** between entities (Persons-Groups, Persons-Recordings)—cross-entity endpoints currently hardcoded with IDs
